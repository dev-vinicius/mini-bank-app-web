...
import { createDebitTransactionAction } from "@/actions/transactions"
...
interface DialogDebitTransactionProps {
    accountId: number
}

export function DialogDebitTransaction({accountId}: DialogDebitTransactionProps) {
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        const result = await createDebitTransactionAction(accountId, Number(data.value))
        if (result.success) {
            toast({
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <button className="w-48 flex flex-col gap-2 items-center p-4 bg-blue-400 rounded-xl hover:bg-blue-600">
                <span className="text-white font-semibold">Débito</span>
                <span className="text-white font-semibold">-</span>
            </button>
            </DialogTrigger>
            <DialogContent>
                <DialogHeader>
                <DialogTitle>Débito</DialogTitle>
                <DialogDescription>
